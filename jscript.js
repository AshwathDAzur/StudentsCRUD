...
document.querySelector(".click").addEventListener("click", function () {
  ...
  let checkdata = [];
  let flag = 0;
  checkdata = JSON.parse(localStorage.getItem("StudentData"));
  for (let k = 0; k < checkdata.length; k++) {
    if (studentID == checkdata[k].id) {
      flag = 1;
    }
  }
  ...
  if (flag == 0) {
    ...
    alert("Student ID is already existing!!!");
  }
});
//----------------------------------------------------------------------------------
...
for (var i = 0; i < StudentData.length; i++) {
  ...
  snocell.textContent = StudentData[i].id;
  fnamecell.textContent = StudentData[i].firstName;
  lnamecell.textContent = StudentData[i].lastName;
  dobcell.textContent = StudentData[i].dob;
  emailcell.textContent = StudentData[i].email;
  gradecell.textContent = StudentData[i].grade;
  divisioncell.textContent = StudentData[i].division;
